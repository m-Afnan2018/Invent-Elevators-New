...
const FALLBACK_PROJECTS = [
  { _id: 'f1', title: 'Villa 56, Al Furjan – Dubai',                    slug: null },
  { _id: 'f2', title: 'Villa 06, Al Zeina Island – Abu Dhabi',          slug: null },
  { _id: 'f3', title: 'DECA Properties Head Office – Al Barsha, Dubai', slug: null },
  { _id: 'f4', title: 'DNEC Office – KAFD, Riyadh, Saudi Arabia',       slug: null },
  { _id: 'f5', title: 'Emirates Hills E13 – Dubai',                      slug: null },
  { _id: 'f6', title: 'NEERA Restaurant – Al Habtoor City, Dubai',      slug: null },
  { _id: 'f7', title: 'Villa 09 – Emirates Hills, Dubai',                slug: null },
];

export default function Footer() {
  const [footerProjects, setFooterProjects] = useState(FALLBACK_PROJECTS);

  useEffect(() => {
    getProjects()
      .then(res => {
        const all = extractCollection(res);
        const picked = all.filter(p => p.showInFooter && p.title);
        if (picked.length > 0) setFooterProjects(picked);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <footer className={styles.footer}>
      ...
      <div className={styles.grid}>
        ...
        {/* Last Projects — dynamic, falls back to static list */}
        <div className={styles.column}>
          ...
          <ul className={styles.linkList}>
            {footerProjects.map(p => (
              <li key={p._id}>
                {p.slug ? (
                  <Link href={`/projects/${p.slug}`} className={styles.link}>{p.title}</Link>
                ) : (
                  <span className={styles.link}>{p.title}</span>
                )}
              </li>
            ))}
          </ul>
        ...
        <div className={styles.column}>
          ...
          <div className={styles.socialIcons}>
            <a href="https://wa.me/971585723553" target="_blank" rel="noreferrer" className={styles.socialBtn} aria-label="WhatsApp">
            ...
            <a href="https://www.instagram.com/inventelevatorco/" target="_blank" rel="noreferrer" className={styles.socialBtn} aria-label="Instagram">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
