...
//     const availableText  = data?.availableText  ?? "Available for new projects";
//     const whatsappNumber = data?.whatsappNumber ?? "971500000000";
//     const buttonText     = data?.buttonText     ?? "Let's Talk";
//     const buttonHref     = data?.buttonHref ?? `tel:+${whatsappNumber}`;
...
//                     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={styles.phoneIcon}>
...
export default function BottomBar({ data }: BottomBarProps) {
  const availableText = data?.availableText ?? "Book Free Site Visit Now";
  const whatsappNumber = data?.whatsappNumber ?? "971500000000";
  const buttonText = data?.buttonText ?? "Let's Talk";
